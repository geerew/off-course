<script lang="ts">
	import { LogsIcon, RightChevronIcon } from '$lib/components/icons';
	...
	<Dropdown.Root>
		...
		>
			<div class="flex items-center gap-3">
				<LogsIcon class="size-4 stroke-2" />
		...
		<Dropdown.Content class="w-42" align="start">
			<div class="flex flex-col gap-1">
				<div class="flex flex-row items-center justify-between px-1.5">
					<span class="text-background-primary-alt-1 text-sm font-semibold">Component</span>
					<Button
						variant="ghost"
						class={cn(
				...
				<Dropdown.CheckboxGroup bind:value={selected}>
					{#each logTypes as type}
						<Dropdown.CheckboxItem value={type.value} closeOnSelect={false}>
							{type.label}
