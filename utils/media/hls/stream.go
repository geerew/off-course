...
// GetSegment retrieves a specific segment, starting transcoding if needed
...
// prerareNextSegements starts encoding future segments for video streams
...
// Kill stops all encoding processes
func (ts *Stream) Kill() {
	ts.lock.Lock()
	defer ts.lock.Unlock()
